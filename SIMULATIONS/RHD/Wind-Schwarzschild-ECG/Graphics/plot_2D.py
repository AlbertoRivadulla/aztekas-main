...
# As many levels as colors available in the color map (cmap.N)
if cbar_min == cbar_max:
    levels = np.linspace(cbar_min, 10, cmap.N) # Levels
else:
    levels = np.linspace(cbar_min, cbar_max, cmap.N) # Levels
...
if stream == 1:
    if orientation == 'v':
        ...
        # Data for the left side of the plot
        sx1Inv = np.linspace( -X1.max(), -X1.min(), Nx1 )
        sx2Inv = np.linspace( -X2.max(), -X2.min(), Nx2 )
        guInv = -gu[::1,::-1]
        gvInv =  gv[::1,::-1]

        ax.streamplot( sx1,sx2, gu,gv,density=[1,2], color='k', linewidth=np.sqrt(gu*gu + gv*gv))
        ax.streamplot( sx1Inv, sx2Inv, guInv, gvInv, density=[1,2], color='k', linewidth=np.sqrt(guInv*guInv + gvInv*gvInv))
    ...
    if orientation == 'h':
        ...
        # Data for the lower side of the plot
        sx1Inv = np.linspace( -X1.max(), -X1.min(), Nx1 )
        sx2Inv = np.linspace( -X2.max(), -X2.min(), Nx2 )
        guInv = -gu[::-1,::1]
        gvInv =  gv[::-1,::1]
        ...
        ax.streamplot( sx2Inv, sx1Inv, gvInv, guInv, density=[2,1], color='k', linewidth=np.sqrt(guInv*guInv + gvInv*gvInv))
