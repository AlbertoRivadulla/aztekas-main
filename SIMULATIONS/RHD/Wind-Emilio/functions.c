...
void Mass_Accretion_Rate(double *B)
{
   ...
   /* char name_time[50], name_acc[50],\ */
   /*      name_domain[50], name_theta[50]; */
   /* char ext[10], dat[10]; */
   char name_time[200], name_acc[200],\
        name_domain[200], name_theta[200];
   char ext[100], dat[100];
   int num;
   ...
   double UhatT, Uhatr, Uhatt, Uhatp;
   double Uhracc, Uhtacc, Uhpacc, racc;
   double Urmean[Nx1+1], Utmean[Nx1+1], Upmean[Nx1+1], dV[Nx1+1];
   ...
   if(time >= Mdot_tprint || time >= tmax)
   {
      ...
      for(int i = gc; i <= Nx1-gc; i++)
      {
         for(int j = gc+1; j <= Nx2-gc-0; j++)
         {
            ...
            Vphi = 0.0;//y_rphi*vr + y_thphi*vth + y_phiphi*vphi;
            ...
            if(j == gc+1 || j == Nx2-gc-0)
            {
               ...
               Uhatp = Uhatt + 4.0*M_PI*Uhphi*lapse*dety*dth/2.0;
            ...
            else
            {
               ...
               Uhatp = Uhatt + 4.0*M_PI*Uhphi*lapse*dety*dth;
      ...
      if(time == 0.0)
      {
         char dum1[100] = "cat /dev/null > ";
         strcat(dum1," ");
         strcat(dum1,name_time);
         system(dum1);
         char dum2[100] = "cat /dev/null > ";
         strcat(dum2," ");
      ...
      if(time >= tprint)
      {
         ...
         fprintf(file_domain,"#r,Mdot,Uhatr,Uhatth,Uhatphi\n");
         fprintf(file_domain,"###################################\n");

         for(int i = gc; i <= Nx1-gc; i++)
         {
            fprintf(file_domain,"%e %e %e %e %e\n",grid.X1[i],\
                                                   Mdotmean[i],\
                                                   Urmean[i]/dV[i],\
                                                   Utmean[i]/dV[i],\
                                                   Upmean[i]/dV[i]);
