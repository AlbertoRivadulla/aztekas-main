...
void Mass_Accretion_Rate(double *B)
{
   ...
   if(time >= Mdot_tprint || time >= tmax)
   {
      ...
      for(int i = gc; i <= Nx1-gc; i++)
      {
         for(int j = gc+1; j <= Nx2-gc-0; j++)
         {
            ...
            if(j == gc+1 || j == Nx2-gc-0)
            {
               ...
               Mdot  = Mdot - 2.0*M_PI*rho*Ur*lapse*dety*dth / 2.;
            ...
            else
            {
               ...
               Mdot  = Mdot - 2.0*M_PI*rho*Ur*lapse*dety*dth;
         ...
         else if(r_plus < x1min)
         {
            ...
            racc     = grid.X1[gc+5];
            /* printf("Radius where Mdot is computed: %lf", racc); */
         }
      ...
      /*
       * Print values in Average_vs_Time.dat
       */
      ...
      /*
       * Print values in Acc_vs_Time.dat
       */
